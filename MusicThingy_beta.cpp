...
#ifdef __HAIKU__
    return "/boot/home/config/settings/MusicThingy/favorites.txt";
#else
    // Fallback for other OS or just return empty if you want it Haiku-only
...
#ifdef __HAIKU__
    return "/boot/home/config/settings/MusicThingy";
#else
    return ".config/MusicThingy";
#endif
}

int count_favorites() {
    std::string home = getenv("HOME") ? getenv("HOME") : ".";
    std::ifstream infile(get_favorites_path());
    int lines = 0;
    std::string line;
